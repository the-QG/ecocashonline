...
our server has been successfuly received and committed to the
database.
...
//DATABASE: this format allows you to toggle nicely between your localhost, and online. change appropriately

$user = $_SERVER['SERVER_NAME'] == "localhost" ? "root" : "user_online";

$pass = $_SERVER['SERVER_NAME'] == "localhost" ? "pw_local" : "pw_pnline";

$db = $_SERVER['SERVER_NAME'] == "localhost" ? "db_local" : "db_online";

$public_key = ""; //the public key we give to you

$secret_key = ""; //the secret key we give to you

define('LOG_TABLE', 'loga'); //name of log table, if you dudnt change it

define('SUBMISSIONS_TABLE', 'submissions'); //name of submission table

define('LOG_EVENTS', TRUE); //log errors & requests

define ('DENY_ALL', FALSE); //if u are say maintaining the db and do not want to accept new submissions
...
$apprv_checkq = "SELECT * FROM ".SUBMISSIONS_TABLE." WHERE approval_code = '".mysql_real_escape_string($approval_code, $conn)."' LIMIT 1";
...
$add_submission_q = "INSERT INTO ".SUBMISSIONS_TABLE." (admi, amount, sender, approval_code, new_balance, time_sms_got_in) VALUES ('".$admi."', '".$amount."', '".$sender."', '".$approval_code."', '".$new_balance."', '".$time_sms_got_in."')";
...
function log_dema($req, $conn){
	if (LOG_EVENTS ==true){
		mysql_query("INSERT INTO ".LOG_TABLE."(yaya) VALUES('".mysql_real_escape_string(json_encode($req), $conn)."')", $conn);
	}
	return;	
